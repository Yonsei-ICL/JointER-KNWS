...
class MultiHeadSelection(nn.Module):
    def __init__(self, hyper) -> None:
        ...
        self.BI_tags = list(self.id2bio.values())
        self.BI_tags.remove('<pad>')
        self.BI_tags.remove('O')
        self.I_tags = [key for key in self.BI_tags if key.startswith("I-")]
        self.B_tags = [key for key in self.BI_tags if key.startswith("B-")]
        ...
        if hyper.cell_name == 'gru':
            ...
            self.post_lstm = nn.LSTM(hyper.emb_size,
                                   ...
                                   bidirectional=True,
                                   batch_first=True)
            self.encoder = AutoModel.from_pretrained("/home/kyuhwan/military/pytorch_multi_head_selection_re_KLUE_BERT/lib/preprocessings/klue/bert_base")
        ...
        self.tagger = CRF(len(self.bio_vocab) - 1, batch_first=True) # self.bio_vocab :{'<pad>': 3, 'B': 0, 'I': 1, 'O': 2}
    ...
    def forward(self, sample, is_train: bool) -> Dict[str, torch.Tensor]:
        ...
        if self.hyper.cell_name in ('lstm', 'gru'):
            ...
            raise ValueError('unexpected encoder name!')
        emi = self.emission(o) # [2, 250, 3] (pad가 3) -> [2, 250, 9]
    ...
    def selection_decode(self, text_list, sequence_tags,
        ...
        def find_entity(pos, text, sequence_tags):
            ...
            temp_entity = []
            while sequence_tags[pos] in self.I_tags:
                temp_entity.append(text[pos])
                pos -= 1
                if pos < 0:
                    break
                if sequence_tags[pos] in self.B_tags:
                    temp_entity.append(text[pos])
                    break
