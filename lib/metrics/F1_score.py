...
class F1_ner(F1_abc):
    ...
    @overrides
    def __call__(self, predictions: List[List[str]], gold_labels: List[List[str]], BI_tags):
        for g, p in zip(gold_labels, predictions):

            inter = sum(tok_g == tok_p and tok_g in set(BI_tags)
                        for tok_g, tok_p in zip(g, p))
            bi_g = sum(tok_g in set(BI_tags) for tok_g in g)
            bi_p = sum(tok_p in set(BI_tags) for tok_p in p)
